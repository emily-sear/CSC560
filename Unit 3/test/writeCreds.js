...
TEST_CREADS_TMP_FILE = '/tmp/readerTestCreds.js';
...
function writeCreds(callback) {
    var fs = require('fs');
    fs.writeFileSync(TEST_CREADS_TMP_FILE, 'TEST_USERS = ');
    fs.appendFileSync(TEST_CREADS_TMP_FILE, JSON.stringify(users_array));
    fs.appendFileSync(TEST_CREADS_TMP_FILE, '; module.exports = TEST_USERS;');
    callback(0);
}
